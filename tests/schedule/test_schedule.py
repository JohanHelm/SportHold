...
async def test_many_schedules(all_days_schedule, none_days_schedule):
    date = datetime.today()
    manager = ScheduleManager()
    current_schedules = manager.is_day_in_schedules(
...
async def test_schedule_gen_slots(all_days_schedule):
    date = datetime.today()
    manager = ScheduleManager()
    slots = manager.generate_time_intervals([all_days_schedule], date=date)
...
async def test_schedule_restricted_gen_slots(day_siesta_schedule, all_days_schedule):
    date = datetime.today()
    manager = ScheduleManager()
    slots = manager.generate_time_intervals(
        [all_days_schedule, day_siesta_schedule], date=date
    )
    current_date = date
